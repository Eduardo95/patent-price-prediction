...
pkl_file = open('2185_data_directory\word2vec_64dim_2185.pkl', 'rb')
word_vectors = pickle.load(pkl_file)
pkl_file.close()

pkl_file = open('2185_data_directory\content_one_hot_jieba_2185.pkl', 'rb')
one_hot = pickle.load(pkl_file)
pkl_file.close()

original_data = pd.read_pickle('2185_data_directory\patent_data_2185.pkl')
price_list = [[i] for i in original_data['price_list']]
...
content_vectors = np.array(one_hot)
...
for i in X_train_content_vec_padding:
    X_train_content_matrix.append(np.append(i, empty_vec(601)).reshape(100, 100))
...
for i in X_test_content_vec_padding:
    X_test_content_matrix.append(np.append(i, empty_vec(1253)).reshape(100, 100))
